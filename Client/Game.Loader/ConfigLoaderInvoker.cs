...
namespace ET
{
    ...
    public class GetAllConfigBytes: AInvokeHandler<ConfigLoader.GetAllConfigBytes, ETTask<Dictionary<Type, byte[]>>>
    {
        public override async ETTask<Dictionary<Type, byte[]>> Handle(ConfigLoader.GetAllConfigBytes args)
        {
            ...
            {
                string ct = "cs";
                GlobalConfig globalConfig = ResourcesLoader.Instance.LoadBuiltin<GlobalConfig>("GlobalConfig");
            ...
            else
            {
                ...
                {
                    output[type] =await  ResourcesLoader.Instance.LoadBytes($"Assets/Bundles/Config/{type.Name}.bytes");
                }
            }
